  ...
  return (
    <div className="mb-6 relative w-full overflow-hidden">
      <div
        ...
        className="flex gap-4 sm:gap-5 overflow-x-auto snap-x snap-mandatory scroll-smooth px-1 sm:px-2 pt-6 pb-6 scrollbar-hide w-full"
      >
        {packageSelections.map(({ pkg, selected }) => (
          <div
            key={pkg.id}
            className={`flex-shrink-0 w-[280px] sm:w-[300px] snap-center sm:snap-start rounded-2xl border-2 p-5 flex flex-col transition-all ${pkg.highlight
              ? 'border-amber-400 bg-gradient-to-b from-amber-50 to-white shadow-xl relative scale-100 sm:scale-[1.02] z-10'
              : 'border-border bg-white hover:border-gold-border hover:shadow-md'
              }`}
          >
            {pkg.highlight && (
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-1.5 py-2 sm:py-2.5 rounded-lg text-[11px] sm:text-sm font-semibold text-white transition-all uppercase tracking-wide ${addedPkgId === pkg.id

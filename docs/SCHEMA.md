...
> `field_type` válidos: `'texto'`, `'texto_longo'`, `'numero'`, `'telefone'`, `'select'`, `'checkbox'`, `'data'`, `'upload_imagem'`, `'upload_imagens'`, `'upload_arquivo'`. `select`/`checkbox` usam `options` (array de strings); `checkbox` aceita múltiplas respostas marcadas na mesma pergunta (gravadas juntas em `candidate_answers.value`, separadas por `'; '` — ver `CHECKBOX_DELIM` em `src/components/rh/FormFieldRenderer.tsx`). `texto_longo` é o mesmo texto simples, só muda o input pra textarea (várias linhas) — pra perguntas como "Resumo da atuação do profissional"; `texto` continua sendo input de uma linha só (usado por `nome` e demais textos curtos). `upload_imagens` (plural — pensado pra perguntas de certificados) é o mesmo esquema: até `MAX_MULTI_UPLOAD` (5) imagens por resposta, URLs do R2 gravadas juntas com o mesmo delimitador; upload vai pra pasta `candidates/certificates` no R2 (allowlist em `supabase/functions/upload-product-image/index.ts`).

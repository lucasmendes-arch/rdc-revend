...
## [D-26] inventory (checkout) passa a ser alimentado pela contagem física de Linhares — reversão parcial da D-21

**Data:** 2026-07-24
**Contexto:** D-21 fechou a porta pra qualquer cruzamento entre o módulo de Estoque por loja (`stock_counts`) e `inventory` (fonte do checkout B2B), porque `inventory` só decrescia via venda e só era realimentado pelo sync manual do Google Sheets — nenhuma automação seria segura sem revisar `create-order` junto. O usuário pediu exatamente essa revisão: parar de depender do Sheets e usar a contagem física real como fonte do estoque do checkout. Ponto de atenção levantado durante o planejamento: o checkout despacha os pedidos B2B a partir de Linhares (o CD) — as 4 lojas satélite vendem no varejo local e não têm como despachar um pedido online. Somar o estoque das satélites ao número do checkout inflaria a disponibilidade com estoque que fisicamente não pode ser enviado.
**Decisão:** `confirm_stock_count()` passou a fazer `UPSERT` em `inventory (product_id, quantity)` **somente quando a contagem confirmada é da loja central** (`stores.type='central'`, Linhares) — `quantity` = `stock_count_items.total_units` de cada produto contado (produtos `stock_only`/sem `units_per_box` ficam de fora, mesmo critério já usado pra pular reposição). Loja satélite continua exatamente como na D-21: só gera `replenishment_requests`, nunca toca em `inventory`. O sync do Google Sheets (`sync-google-sheets`) foi desativado na UI de `/admin/estoque` (a edge function continua no repo, sem uso). `create-order/index.ts` **não foi alterado** — sequência de execução, `decrement_stock`, invariantes, tudo igual; só mudou o que alimenta `inventory.quantity` de tempos em tempos.
**Consequência:** Entre uma contagem de Linhares e outra, `inventory.quantity` segue decrescendo normalmente a cada pedido (`decrement_stock`, inalterado); a próxima contagem de Linhares confirmada faz um novo corte, substituindo o valor acumulado pelo total físico real (mesma lógica de auditoria periódica que já existia — só a fonte do "reabastecimento" mudou de Sheets pra contagem física). Editar `quantity` manualmente em `/admin/estoque` continua possível como ajuste pontual, mas é sobrescrito na próxima contagem de Linhares. Produto novo, nunca contado em Linhares e sem linha em `inventory`, continua tratado como estoque ilimitado pelo `create-order` — comportamento pré-existente, não alterado aqui. Se nenhuma contagem de Linhares estiver confirmada no momento do deploy, `inventory` fica com o último valor do Sheets até a primeira contagem de Linhares confirmada depois — recomendável confirmar uma logo após subir a mudança.

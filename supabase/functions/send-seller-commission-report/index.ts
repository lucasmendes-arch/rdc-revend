...
function fmtDate(iso: string): string {
  // Strings YYYY-MM-DD não devem passar pelo construtor Date (seria interpretado
  // como UTC midnight e ao converter para BRT (UTC-3) vira o dia anterior)
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ── Parse body ──────────────────────────────────────────────────
    const { seller_id, start_date, end_date } = await req.json()
    if (!seller_id || !start_date || !end_date) {
      return json({ error: 'seller_id, start_date e end_date são obrigatórios' }, 400, req)
    }

    // ── RPC ─────────────────────────────────────────────────────────
    // Chamada com userClient para que auth.uid() esteja disponível
    // e is_admin() funcione dentro da RPC (SECURITY DEFINER)
    const { data: summary, error: rpcErr } = await userClient.rpc(
      'get_seller_commission_summary',
      { p_seller_id: seller_id, p_start_date: start_date, p_end_date: end_date },
    )
    if (rpcErr) {
      console.error('RPC error:', rpcErr.message)
      return json({ error: rpcErr.message }, 500, req)
    }

    const data = summary as CommissionData
    if (data.summary.total_orders === 0) {
      return json({ error: 'Nenhum pedido finalizado encontrado para este vendedor no período' }, 404, req)
    }
    ...
    if (uploadErr) {
      ...
      return json({ error: 'Erro ao fazer upload do PDF' }, 500, req)
    }
    ...
    if (urlErr || !urlData?.signedUrl) {
      console.error('Signed URL error:', urlErr?.message)
      return json({ error: 'Erro ao gerar URL do PDF' }, 500, req)
    }
    // ── Encurtar URL via TinyURL (sem auth, free) ───────────────────
    ...
    try {
      ...
      console.warn('TinyURL falhou — usando URL original')
    }
    ...
    if (uazapiUrl && uazapiToken) {
      ...
      const message = [
        ...
        `📎 PDF: ${pdfUrl}`,
        ``,
        `_Gerado pelo sistema RDC Revend_`,
      ].join('\n')
      ...
      if (!wppRes.ok) {
        ...
        console.warn('WhatsApp send failed (non-blocking):', wppErr)
      } else {
        console.log('WhatsApp sent to', destNumber)
      }
    }

    return json({ success: true, pdf_url: pdfUrl, summary: data.summary, seller: data.seller }, 200, req)

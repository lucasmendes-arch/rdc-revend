-- Adiciona p_shipping a create_manual_order para que o frete seja persistido
-- e somado ao total em pedidos com entrega normal.
...
BEGIN
  ...
  -- total = subtotal + frete - desconto (nunca negativo)
  v_total := GREATEST(v_subtotal + v_shipping - COALESCE(p_discount, 0), 0);
  ...
  INSERT INTO orders (
    ...
    p_user_id, p_status, v_subtotal, v_shipping, v_total,
...
REVOKE EXECUTE ON FUNCTION public.create_manual_order(
  uuid, jsonb, numeric, text, text, text, text, numeric, uuid, timestamptz, uuid, numeric
) FROM PUBLIC;

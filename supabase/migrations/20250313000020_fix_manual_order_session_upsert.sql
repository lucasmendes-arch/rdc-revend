...
-- Correção do erro 23505: ON CONFLICT (session_id) não cobria a constraint
-- UNIQUE (user_id) adicionada em _014. Trocado para ON CONFLICT (user_id).
-- ============================================================================
...
BEGIN
  ...
  -- session_id é atualizado para o padrão canônico user_{uuid} se necessário.
...
REVOKE EXECUTE ON FUNCTION public.create_manual_order(uuid,jsonb,numeric,text,text,text,text,numeric,uuid,timestamptz) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.create_manual_order(uuid,jsonb,numeric,text,text,text,text,numeric,uuid,timestamptz) TO authenticated;

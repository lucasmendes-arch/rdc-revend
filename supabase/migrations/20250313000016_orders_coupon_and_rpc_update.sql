...
-- 1. Adicionar coupon_id em orders (FK → coupons)
-- 2. Recriar create_manual_order com p_coupon_id + incremento de used_count
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. Coluna coupon_id em orders
-- ----------------------------------------------------------------------------
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS coupon_id UUID REFERENCES public.coupons(id);

-- ----------------------------------------------------------------------------
-- 2. Recriar create_manual_order — assinatura com 9 params + p_coupon_id
--    Mantém toda a lógica existente (subtotal, CRM, client_sessions, etc.)
-- ----------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.create_manual_order(uuid,jsonb,numeric,text,text,text,text,numeric);
...
REVOKE EXECUTE ON FUNCTION public.create_manual_order(uuid,jsonb,numeric,text,text,text,text,numeric,uuid) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.create_manual_order(uuid,jsonb,numeric,text,text,text,text,numeric,uuid) TO authenticated;
